...
import ProductInput, { DiscountInput } from '@/public/components/CMS/ProductInput';
import useSWR from 'swr';
import { getCookie } from 'cookies-next';
import { Product, ProductInput as ProductInputType } from '@/public/types/product';
import { validateField } from '@/public/validation/productValidation';
import { GET_PRODUCTS, UPDATE_PRODUCT } from '@/public/graphql/productQueries';
                            ...
                            <div className="overflow-x-auto scrollable-section relative">
                                <div className="flex gap-4 pb-4 w-max">
                                    ...
                                    <div className="flex flex-col gap-2 w-20 sm:w-46">
                                        <label className="text-xs sm:text-sm text-gray-700 text-shadow">افزودن لینک داخلی</label>
                                        <div className="flex gap-1">
                                            <div className="flex-1">
                                                <InternalLinkSelector
                                                    ...
                                                    placeholder="مقاله..."
                                                    onSelect={(link) => {
                                                        const currentDesc = editingProducts[product._id]?.desc ?? initialProductValues?.desc ?? '';
                                                        handleFieldChange(product._id, 'desc', currentDesc + '\n' + link);
                                                    }}
                                                />
                                            </div>
                                            <div className="flex-1">
                                    ...
                                    <div className="flex flex-col gap-2 w-20 sm:w-46">
                                        <label className="text-xs sm:text-sm text-gray-700 text-shadow">لینک اینستاگرام</label>
                                        <input
                                            type='text'
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const currentDesc = editingProducts[product._id]?.desc ?? initialProductValues?.desc ?? '';
                                                const productTitle = editingProducts[product._id]?.title ?? initialProductValues?.title ?? product.title;
