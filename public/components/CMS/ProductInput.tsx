        ...
        <div className={`flex flex-col gap-2 ${form ? "w-full" : " w-20 sm:w-48"}`}>
            ...
            {type === 'textarea' ? (
                <textarea
                    ...
                    onBlur={handleSave}
                    onFocus={onFocus}
                    className={`rounded-lg p-1.5 sm:p-2 border bg-slate-50 text-xs sm:text-sm focus:outline-none resize-y min-h-25 w-full ${error ? 'border-red-500' : 'border-gray-300'}`}
                    rows={4}
                    autoFocus
                />
            ) : type === 'select' ? (
