...
type InputProps = {
    ...
    autoFocus?: boolean
    box?: boolean
    type?: "input" | "select" | "textarea" | "radio" | "instagram-link" | "internal-link"
    placeholder?: string
...
const Input = memo(({ id, form, disabled, required, label, placeholder, setForm, type = "input", options, autoFocus, descForm, titleForm, box }: InputProps) => {
                ...
                <div className="relative">
                    ...
                    {(type === "instagram-link" || type === "internal-link") &&
                        <button
                            ...
                            className={`${(disabled || !form.value) ? "cursor-not-allowed" : "cursor-pointer"} text-xs px-3 sm:py-2 py-1.5 rounded-md bg-slate-600/80 text-neutral-200 hover:bg-slate-600 transition absolute sm:top-5 top-4 -translate-y-1/2 left-[1%]`}
