...
// تغییر کوئری اضافه کردن به سبد خرید بر اساس بک جدید + افزودن پکیج و محصول
// تغییر فرمت اطلاع رسانی در صورت موجود شدن محصول

function BuyBtn({ showCount, id, fix = false, cat, price, state, isPackage }: BuyBtnProps) {
    ...
    const addLocalBascket = async () => {
        ...
        const productId = isPackage ? undefined : id
        const packageId = isPackage ? id : undefined
    ...
    const handleAddToBasket = async (): Promise<void> => {
        ...
        if (jwt) {
            ...
            const variable = isPackage ? { packageId: id, count: 1 } : { productId: id, count: 1 }
            try {
        ...
        <div className={`relative group`}>
            ...
            {state === "callForPrice" ?
                !fix && (
                    <Link
                        href={"#footer"}
                        className="transition-all flex justify-center items-center duration-75 active:border-white bg-black border-mist-700 hover:bg-slate-900 py-2.5 w-full rounded-lg text-white border-b-4 border-solid active:translate-y-1 mt-4"
